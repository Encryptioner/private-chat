...
//
// lockModelSelector: a site-owner modelUrl is active (and downloadable). In that
// case the picker + uploader are dropped entirely — a visitor must not see any
// model name. If the site owner set an embed `label`, show THAT in the header
// instead (a branded title); otherwise render nothing.
function ChatHeader({
  selectedModel,
  isBusy,
  onToggleSidebar,
    ...
    <Flex direction="row" align="center" justify="between" asChild>
      <header>
        <Flex gap="4" align="center">
          ...
          {lockModelSelector ? (
            // No model picker — just the site's embed label as a quiet title, if set.
            widgetLabel ? (
...
ChatHeader.propTypes = {
  ...
  lockModelSelector: PropTypes.bool,
