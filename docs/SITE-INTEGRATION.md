...
<script>
  window.PRIVATE_CHAT_CONFIG = {
    ...
    modelUrl: null,                // optional — load your own GGUF; hides the picker while it loads (falls back if it can't)
...
| `modelUrl` | `string` | URL to a GGUF file to load instead of the built-in default model — any public preset, or one you trained/fine-tuned yourself. Must be hosted with CORS enabled (HF Spaces, R2, GitHub raw, S3 with a CORS policy, etc.). See [`docs/CUSTOM-MODEL-TRAINING.md`](CUSTOM-MODEL-TRAINING.md). **While set and downloadable, the model picker and GGUF uploader are hidden** — a visitor only ever sees *your* model, never the built-in presets. If the URL can't be downloaded (bad URL, missing CORS, 404), the widget silently falls back to the built-in default and reveals the standard picker so the visitor can still pick a model. |
