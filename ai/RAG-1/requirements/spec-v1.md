...
| 2026-08-03 | NFR Security / R6 | R6 scoped the XSS check to `Markdown.jsx`/`rehype-raw` (clean). Implementation surfaced a second path R6 missed: `formatMessageContent` rendered unsanitized model output via `dangerouslySetInnerHTML`. RAG amplifies it (scraped text → model context → echoed → live). Fixed by escaping HTML first (`src/lib/formatMessage.js`) before the code-block/URL wrappers; both render branches now safe. Fulfills the NFR Security intent. | Discovered during Story 6 implementation; security-critical, fixed in-place rather than deferred. |
